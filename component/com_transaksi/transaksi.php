        ...
        <div class="row">
            <div class="col-md-12">
                <div class="x_panel">
                    ...
                    <div>
                        <h4>
                            <small>Saldo Keseluruah : 
                            </small>Rp. <?php echo rupiah($saldo_keseluruhan);?>
                        </h4>
                    </div>
                    <div class="x_content">                    
...
if (empty($_POST['kredit'])) {
    ...
    $saldo = mysqli_fetch_array($query_saldo);
    $saldoo= $saldo['jumlah_debit'] - $saldo['jumlah_kredit'];
    $sisasaldo= $saldoo - $_POST[kredit];
      if ($saldoo <= 20000  ){
        echo "<script>window.alert('Saldo anda kurang dari Rp.20.000 !')
    window.location='?module=transaksi'</script>";
      }elseif($saldoo <= $_POST[kredit]){
         echo "<script>window.alert('Saldo anda tidak cukup !')
    window.location='?module=transaksi'</script>";
      }elseif( $sisasaldo < 20000){
        echo "<script>window.alert('Saldo tidak boleh kurang dari Rp.20.000 !')
    window.location='?module=transaksi'</script>";
      }else{
    ...
    mysqli_query($conn,"UPDATE nasabah SET saldo = '$sisasaldo'
                                    WHERE id_nasabah = '$_POST[id_nasabah]'");
...
<?php          
    $query_trans=mysqli_query($conn,"SELECT SUM(jumlah) as jml_trans FROM transaksi_k");
    $row_jumlah=mysqli_fetch_array($query_trans);
    $total_trans= $row_jumlah['jml_trans'];
?>
<!-- page content -->
<div class="col" role="main">
    <div class="">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="x_panel">
                    <div class="x_title" style="text-transform: capitalize;">
                        <h2 >Data <?php echo $_GET['module'];?> Kantin</h2>
                        <div class="clearfix"></div>
                    </div>                   
                    <!-- <div>
                        <h4>
                            <small>Total Transaksi : 
                            </small>Rp. <?php //echo rupiah($total_trans);?>
                        </h4>
                    </div> -->
                    <div class="x_content">                    
                        <table id="datatable" class="table table-bordered">
                            ...
                            <tbody>
                                <?php 
                                  $no = 0;
                                  $query=mysqli_query($conn,"SELECT * FROM transaksi_k JOIN nasabah ON transaksi_k.id_nasabah=nasabah.id_nasabah ORDER BY id_transaksi DESC");
                                  $count = 2 ;
                                  while($row=mysqli_fetch_array($query)){
                                  $no++;
                                ?>                                  
